...
$sections['featured-communities']      =   get_post_meta(get_the_ID(), 'inspiry_show_home_news_modern', true); // Featured Communities.
...
if (!empty($section_ordering) && $section_ordering == 'default') {
	$home_sections = 'content,latest-properties,featured-properties,testimonial,cta,agents,features,partners,news,cta-contact, featured-communities';
	$home_sections = explode(',', $home_sections);
} else {
	$home_sections = get_post_meta(get_the_ID(), 'inspiry_home_sections_order_mod', true);
	$home_sections = (!empty($home_sections)) ? $home_sections : 'content,latest-properties,featured-properties,testimonial,cta,agents,features,partners,news,cta-contact, featured-communities';
	$home_sections = explode(',', $home_sections);
}
// PIDHomes:: insert featured-communities
array_splice($home_sections, 2, 0, 'featured-communities');
