...
if (function_exists('putRevSlider') && (!empty($rev_slider_alias))) {
    ...
    $_banner_test = get_field('page_banner_background_image');
    if ($_banner_test) {
    ...
    if (empty($banner_title)) {
        ...
        $community_label = get_query_var('community_label');
        if ($community_label) {
            $banner_title = $community_label . ' Communities';
        } else {
            $rps_city = get_query_var('input_city');
            if ($rps_city) {
                $banner_title = ucfirst($rps_city) . " PID Homes";
            } else {
                $banner_title = "Greater Vancouver PID Homes";
            }
        }
    }
