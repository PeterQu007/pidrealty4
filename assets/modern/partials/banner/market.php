...
if (function_exists('putRevSlider') && (!empty($rev_slider_alias))) {
  ...
  $banner_image_test = get_field('page_banner_background_image');
  if ($banner_image_test) {
  ...
  if (empty($banner_title)) {
    ...
    $community_label = $env->community_label;
    if ($community_label) {
      if ($post_type == 'market') {
        $banner_title = $community_label . ($post_lang === 'cn' ? '地产市场概况' : ' Real Estate Market Reports');
      } else {
        $banner_title = $community_label . ($post_lang === 'cn' ? '房价比较市场报告' : ' Comparables Market Report');
      }
    } else {
      $banner_title = $post_lang === 'cn' ? "大温地区地产市场概况" : "Greater Vancouver Real Estate Market Reports";
    }
  }
